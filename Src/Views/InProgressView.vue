...
const filteredTasks = computed(() =>
  tasksStore.tasks.filter((t) => t.status === 'inprogress')
)
  ...
  <div class="flex min-h-screen">
    ...
    <main class="ml-56 flex-1 bg-[#F4F5F7] dark:bg-[#0a1628] px-6 py-8 lg:px-10">
      ...
      <div class="mb-6 flex items-end justify-between">
        <div>
          <h1 class="text-xl font-bold text-[#0E1F3D] dark:text-white">⚡ In Progress <span class="ml-1 text-base font-normal text-slate-400">({{ filteredTasks.length }})</span></h1>
          <p class="mt-0.5 text-sm text-slate-500 dark:text-slate-400">Tasks currently being worked on.</p>
        </div>
      ...
      <div v-if="filteredTasks.length === 0" class="mt-20 text-center">
        ...
        <p class="text-lg font-semibold text-[#0E1F3D] dark:text-slate-300">Nothing in progress!</p>
        <p class="text-sm text-slate-400 mt-1">Move a task from To Do to get started.</p>
      </div>
    </main>
