...
function handleDelete(id) { tasksStore.deleteTask(id) }
  ...
  <div class="flex min-h-screen">
    ...
    <main class="ml-56 flex-1 bg-[#F4F5F7] dark:bg-[#0a1628] px-6 py-8 lg:px-10">
      ...
      <div class="mb-6 flex items-end justify-between">
        <div>
          <h1 class="text-xl font-bold text-[#0E1F3D] dark:text-white">✅ Completed <span class="ml-1 text-base font-normal text-slate-400">({{ filteredTasks.length }})</span></h1>
          <p class="mt-0.5 text-sm text-slate-500 dark:text-slate-400">Tasks you've finished. Great work!</p>
        </div>
      ...
      <section class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        <TaskCard v-for="task in filteredTasks" :key="task.id" :task="task" :show-delete="true" @status-change="handleStatusChange" @delete="handleDelete" @share="handleShare" />
      </section>
      <div v-if="filteredTasks.length === 0" class="mt-20 text-center">
        <p class="text-5xl mb-4">✅</p>
        <p class="text-lg font-semibold text-[#0E1F3D] dark:text-slate-300">No completed tasks yet.</p>
        <p class="text-sm text-slate-400 mt-1">Mark tasks as done to see them here.</p>
      </div>
    </main>
